...
(function () {
    ...
    /* JAMAAH CALC */
    var jamaahCalc = function jamaahCalc(num, time, timenext) {
    ...
    /* PRAYER CONSTRUCTOR */
    var Prayer = function Prayer(now, num) {

        // DST settings
        var dst;
        var dstcheck = moment.tz(moment().add(tomorrow, "day"), city).isDST();
        if (!dstcheck && moment().format("M") == "10") dst = -1;else if (dstcheck && moment().format("M") == "3") dst = 1;else dst = 0;
    ...
    /* TIME DISPLAY */
    var timeDisp = function timeDisp() {
        ...
        for (var i = 0, list = settings.names; i < 6; i++) {
            ...
            if (!hidejamaah) {
                // if hidejamaah is not set in mobile.pug
        ...
        if (document.getElementById("overlay")) {
            ...
            // Taraweeh Prayer
            if (hijriMonth == '9' && (moment().isBetween(moment({
                hour: '0',
                minute: '00'
            }), moment({
                hour: '0',
            ...
            //  (((moment().isBetween(moment({hour: def.isha.jamaahtime.format('H'), minute: def.isha.jamaahtime.format('m')}), moment().endOf('day').add(35, 'minutes')))))
                ...
                // Friday Prayer
            else if (moment().format("d") == '5' && moment().isBetween(moment({
                    hour: jummuahtime[0],
    ...
    function appendZero(unit) {
        var unit;
        if (unit < 10) unit = '0' + unit;else unit = unit;
        return unit;
    }
})();